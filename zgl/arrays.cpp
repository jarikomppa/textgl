...
glopArrayElement(GLContext *c, GLParam *param)
{
  ...
  if (states & COLOR_ARRAY) {
    ...
    p[4].f = size > 3 ? c->color_array[i+3] : (float)(1);
    glopColor(c, p);  
  }
  ...
  if (states & TEXCOORD_ARRAY) {
    ...
    c->current_tex_coord.z = size > 2 ? c->texcoord_array[i+2] : (float)(0);
    c->current_tex_coord.w = size > 3 ? c->texcoord_array[i+3] : (float)(1);
  }
  if (states & VERTEX_ARRAY) {
    ...
    p[3].f = size > 2 ? c->vertex_array[i+2] : (float)(0);
    p[4].f = size > 3 ? c->vertex_array[i+3] : (float)(1);
    glopVertex(c, p);
  }
}
